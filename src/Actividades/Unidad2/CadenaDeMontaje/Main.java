...
public class Main extends JFrame implements WindowListener {
	...
	private final Colocador colocador;
	private final Empaquetador[] empaquetadors;
	...
	public Main() {
		...
		int n = 20;
		cadena = new Cadena(n);
		colocador = new Colocador(cadena);
		colocador.start();
		empaquetadors = new Empaquetador[4];
		for (int i = 0; i < 4; i++) {
			empaquetadors[i] = new Empaquetador(cadena, i+1);
			empaquetadors[i].start();
		}
	...
	private void pausa(ActionEvent e) {
		...
		cadena.pausar();
		colocador.pausar();
		for (Empaquetador empaquetador : empaquetadors) {
			empaquetador.pausar();
		}
